...
entity qaoa_kernel_control_s_axi is
generic (
...
end entity qaoa_kernel_control_s_axi;
...
architecture behave of qaoa_kernel_control_s_axi is
    ...
    constant ADDR_GAMMA_DATA_0          : INTEGER := 16#18#;
    constant ADDR_GAMMA_CTRL            : INTEGER := 16#1c#;
    constant ADDR_BETA_DATA_0           : INTEGER := 16#20#;
    constant ADDR_BETA_CTRL             : INTEGER := 16#24#;
    constant ADDR_GET_BEST_STATE_DATA_0 : INTEGER := 16#28#;
    constant ADDR_GET_BEST_STATE_CTRL   : INTEGER := 16#2c#;
    constant ADDR_BEST_STATE_DATA_0     : INTEGER := 16#30#;
    constant ADDR_BEST_STATE_CTRL       : INTEGER := 16#34#;
    constant ADDR_D_BASE                : INTEGER := 16#40#;
    constant ADDR_D_HIGH                : INTEGER := 16#7f#;
    ...
    signal int_gamma           : UNSIGNED(31 downto 0) := (others => '0');
    signal int_beta            : UNSIGNED(31 downto 0) := (others => '0');
    signal int_get_best_state  : UNSIGNED(0 downto 0) := (others => '0');
    signal int_best_state_ap_vld : STD_LOGIC;
    signal int_best_state      : UNSIGNED(31 downto 0) := (others => '0');
    -- memory signals
    signal int_d_address0      : UNSIGNED(3 downto 0);
    signal int_d_ce0           : STD_LOGIC;
    signal int_d_q0            : UNSIGNED(31 downto 0);
    signal int_d_address1      : UNSIGNED(3 downto 0);
    ...
    component qaoa_kernel_control_s_axi_ram is
        generic (
    ...
    end component qaoa_kernel_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_d
int_d : qaoa_kernel_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_d_read = '0') else '0';
    RVALID    <= RVALID_t;
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_GAMMA_DATA_0 =>
                        rdata_data <= RESIZE(int_gamma(31 downto 0), 32);
                    when ADDR_BETA_DATA_0 =>
                        rdata_data <= RESIZE(int_beta(31 downto 0), 32);
                    when ADDR_GET_BEST_STATE_DATA_0 =>
                        rdata_data <= RESIZE(int_get_best_state(0 downto 0), 32);
                    when ADDR_BEST_STATE_DATA_0 =>
                        rdata_data <= RESIZE(int_best_state(31 downto 0), 32);
                    when ADDR_BEST_STATE_CTRL =>
                        rdata_data(0) <= int_best_state_ap_vld;
                    when others =>
                        NULL;
                    end case;
...
-- ----------------------- Register logic ----------------
    ...
    get_best_state       <= STD_LOGIC_VECTOR(int_get_best_state);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_gamma(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_GAMMA_DATA_0) then
                    int_gamma(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_gamma(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_get_best_state(0 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_GET_BEST_STATE_DATA_0) then
                    int_get_best_state(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_get_best_state(0 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_best_state <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (best_state_ap_vld = '1') then
                    int_best_state <= UNSIGNED(best_state);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_best_state_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (best_state_ap_vld = '1') then
                    int_best_state_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_BEST_STATE_CTRL) then
                    int_best_state_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- d
    int_d_address0       <= UNSIGNED(d_address0);
    int_d_ce0            <= d_ce0;
    d_q0                 <= STD_LOGIC_VECTOR(RESIZE(int_d_q0, 32));
    int_d_address1       <= raddr(5 downto 2) when ar_hs = '1' else waddr(5 downto 2);
    int_d_ce1            <= '1' when ar_hs = '1' or (int_d_write = '1' and WVALID  = '1') else '0';
    int_d_we1            <= '1' when int_d_write = '1' and w_hs = '1' else '0';
    int_d_be1            <= UNSIGNED(WSTRB) when int_d_we1 = '1' else (others=>'0');
...
entity qaoa_kernel_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity qaoa_kernel_control_s_axi_ram;

architecture behave of qaoa_kernel_control_s_axi_ram is
